...
For LIME, the surrogate model coefficients are used as instance-level contributions.
For MAPLE, each coefficient $\beta_i$ is multiplied by the corresponding feature
value to obtain $\phi_i = \beta_i \cdot x_i$. All three return weights and
intercepts in the same format expected by CCC.
...
\begin{table*}[t]
  ...
  \texttt{results\_20260629\_105230.md}. Lower is better. (K=4 excluded per Miller's cognitive limit focus.)}
  ...
  \begin{tabular}{llrrrrr}
    ...
    Adult/XGB & MAPLE & 0.0180 & 0.0180 & 0.0179 & 0.0179 & 0.0179 \\
...
\begin{table}[t]
  ...
  \begin{tabular}{p{0.22\columnwidth}p{0.22\columnwidth}p{0.22\columnwidth}p{0.22\columnwidth}}
    \toprule
    Metric & Quantity measured & What it misses & How CCC differs \\
    \midrule
    Full MSE/local accuracy & All-feature reconstruction & Doesn't evaluate truncated explanation & Measures top-$K$ reconstruction \\
    Sufficiency & Top-$K$ features as perturbed inputs & Depends on masking baseline & Evaluates additive concordance directly \\
    Comprehensiveness & Prediction change when removing features & Perturbation effect, not additive form & Measures local concordance \\
    Infidelity/sensitivity & Stability under input perturbations & No cognitive budget constraint & Explicitly budget-conditioned \\
    Conciseness & How short the explanation is & Doesn't measure faithfulness & Unites compactness and faithfulness \\
    \bottomrule
  \end{tabular}
\end{table}
